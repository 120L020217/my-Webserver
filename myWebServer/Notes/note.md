...
#### IO模型中，同步异步、阻塞非阻塞
![好文](https://cloud.tencent.com/developer/article/1684951)
...
#### 信号量既可用于线程间通信，又可用在进程间通信
在 POSIX 系统中，信号量是一种同步机制，通常用于控制对共享资源的访问。信号量可以是内核空间中的变量，也可以是用户空间中的变量，具体取决于信号量的类型。  

System V 信号量：System V IPC（Inter-Process Communication）提供了一种类型的信号量，它是由内核管理的，存储在内核空间中。System V 信号量通过系统调用来进行操作，如 semget()、semop() 等。这种信号量用于进程间通信，而不是线程间通信。  
POSIX 信号量：POSIX 线程（pthread）提供了一种类型的信号量，它是用户空间中的变量。POSIX 信号量通过 pthread 库提供的函数来进行操作，如 sem_init()、sem_wait()、sem_post() 等。这种信号量通常用于线程间通信，而不是进程间通信。  
总之，信号量是一个抽象概念，在具体实现中，既可以在用户区，也可以在内核区，他们的共性是只有一个进程（线程）能进入改变信号量。
